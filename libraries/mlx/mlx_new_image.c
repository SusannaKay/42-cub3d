...
void	*mlx_int_new_p_xshm_image(t_xvar *xvar,int width,int height,int format)
{
  t_img	*img;
  int	(*save_handler)();
...
void	*mlx_new_image(t_xvar *xvar,int width, int height)
{
  ...
  if (xvar->use_xshm)
    if (img = mlx_int_new_p_xshm_image(xvar,width,height,ZPixmap))
      return (img);
...
void	*mlx_new_image2(t_xvar *xvar,int width, int height)
{
  ...
  if (xvar->use_xshm)
    if (img = mlx_int_new_p_xshm_image(xvar,width,height,XYPixmap))
      return (img);
